...
class Predator(Animal):
    maxHunger = 1200  # The maximum amount of food the predator can store.
    ...
    def __init__(self, nLatticeLength, x=None, y=None, visibilityRadius=15, reproductionRate=0.015,child=False, randomTurnProbability=0.3):
        ...
        if child:
            self.hunger = round(Predator.maxHunger / 2)  # Children start out with semi-full hunger bar.
        else:
            self.hunger = 950  # The initial predator
    ...
    def searchPrey(self):
        ...
        if self.hunger < 950: # If recently eaten will not move
